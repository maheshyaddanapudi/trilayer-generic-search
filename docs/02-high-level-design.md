...
|---|---|---|
| `FinancialMetadataPlugin` | Accounts, Versions, Sheets, Dimensions | XML file / Adaptive Planning API |
...
| **Live API responses** | A connector fetches real-time data (e.g., current account balance from Adaptive Planning) mid-query; the response is wrapped as a `SessionChunk` and injected |
